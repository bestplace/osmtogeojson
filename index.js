...
osmtogeojson = function (json, options) {
    ...
    var context = {
        ...
        pois: new Array(),
        relids: new Array(),
        relsmap: { node: {}, way: {}, relation: {} },

        featuresChunk: new Array(),
    }


    function featureCallback(feature) {
        feature = flattenFeature(feature);
        context.featuresChunk.push(feature);
    }

    return function(contextCallback) {
        if (context.elements.length) {
            return _regroupElements(context, contextCallback)
        }

        if (context.stage.name != 'done') {
            return _convert2geoJSON(context, options, contextCallback, featureCallback);
        }

        contextCallback(context, null, 0);
    }
}

function flattenFeature(feature) {
    feature.properties = _.merge(
...
function chunkedProcessStage(array, context, itemCallback, contextCallback) {
    ...
    return context.stage.done;
}


function _regroupElements(context, contextCallback) {
    var nodes = context.nodes;
    var ways = context.ways;
    var rels = context.rels;
    ...
    CHUNK_SIZE = context.elementsTotal / 20;
    ...
    context.stage.total = context.elementsTotal;
    context.stage.processed = context.elementsProcessed;
    contextCallback(context, j, context.elementsTotal)
}


function newStage(name) {
    return {name, processed: 0}
}


function nextStageIfDone(context, name) {
    if (context.stage.done) {
        context.stage = newStage(name);
    }
}


function _convert2geoJSON(context, options, contextCallback, featureCallback) {
    var nodes = context.nodes;
    var rels = context.rels;
    var ways = context.ways;
    ...
    if (context.stage.name == 'elements') {
        context.stage = newStage('processNodes')
    }

    if (context.stage.name == 'processNodes') {
        ...
        return nextStageIfDone(context, 'preprocessRels')
    }


    if (context.stage.name == 'preprocessRels') {
        var poinids = context.poinids;
        // todo -> after deduplication of relations??
        chunkedProcessStage(context.rels, context, function(i, l) {
        ...
        return nextStageIfDone(context, 'preprocessWays');
    }


    if (context.stage.name == 'preprocessWays') {
        var wayids = context.wayids;
        var waynids = context.waynids;
        var nodeids = context.nodeids;
        chunkedProcessStage(context.ways, context, function(i, l) {
            var way = ways[i];
            if (wayids[way.id]) {
                // handle input data duplication
            ...
            if (_.isArray(way.nodes)) {
                for (var j = 0; j < way.nodes.length; j++) {
                    if (typeof way.nodes[j] === "object") return; // ignore already replaced way node objects
        ...
        return nextStageIfDone(context, 'nodeids');
    }

    if (context.stage.name == 'nodeids') {
        var nodeids = context.nodeids;
        var waynids = context.waynids;
        var poinids = context.poinids;
        for (var id in nodeids) {
            ...
            if (!waynids[id] || poinids[id])
                context.pois.push(node);
        }
        context.stage = newStage('deduplicate rels')
    }
    
    if (context.stage.name == 'deduplicate rels') {
        var rels = context.rels;
        var relids = context.relids;
        chunkedProcessStage(rels, context, function(i, l) {
            var rel = rels[i];
            if (relids[rel.id]) {
                // handle input data duplication
        ...
        return nextStageIfDone(context, 'relids')
    }

    if (context.stage.name == 'relids') {
        var relids = context.relids;
    ...
    if (context.stage.name == 'pois') {
        ...
        return nextStageIfDone(context, 'rels')
    }

    if (context.stage.name == 'rels') {
        var rels = context.rels;
        var relids = context.relids;
        var wayids = context.wayids;
        var relsmap = context.relsmap;
        ...
        chunkedProcessStage(rels, context, function(i, l) {
            ...
            if ((typeof rels[i].tags != "undefined") &&
                ...
                if (!_.isArray(rels[i].members)) {
                    ...
                    return; // ignore relations without members (e.g. returned by an ids_only query)
                }
                ...
                if (feature === false) {
                    ...
                    return; // abort if feature could not be constructed
                }
            ...
            if ((typeof rels[i].tags != "undefined") &&
                ...
                if (outer_count == 0) {
                    ...
                    return; // ignore multipolygons without outer ways
                }
                var simple_mp = false;
                ...
                if (!simple_mp) {
                    ...
                    if (outer_way === undefined) {
                        ...
                        return; // abort if outer way object is not present
                    }
        ...
        return nextStageIfDone(context, 'ways')
    }
    if (context.stage.name == 'ways') {
        var ways = context.ways;
        var wayids = context.wayids;
        var relsmap = context.relsmap;
        // process lines and polygons
        chunkedProcessStage(ways, context, function(i, l) {
            ...
            if (!_.isArray(ways[i].nodes)) {
                ...
                return; // ignore ways without nodes (e.g. returned by an ids_only query)
            }
            if (ways[i].is_skippablerelationmember)
                return; // ignore ways which are already rendered as (part of) a multipolygon
            ...
            if (typeof ways[i].nodes[0] != "undefined" && typeof ways[i].nodes[ways[i].nodes.length - 1] != "undefined" && // way has its start/end nodes loaded
                ...
                (
                    ...
                    _isPolygonFeature(ways[i].tags, options) // ... and tags say it is a polygon
        ...
        return nextStageIfDone(context, 'done')
    }
}
